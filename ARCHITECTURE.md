...
graph TB
    ...
    subgraph BACKEND["后端核心层 (DDD 四层架构)"]
        subgraph PROXY_MOD["代理服务器模块 (axum 0.7)"]
            ROUTE["通配路由匹配"]
            CLIENT_AUTH["客户端鉴权<br/>(可选, 按接入点独立配置)"]
            REMOTE_AUTH["远程认证头注入<br/>Anthropic: x-api-key<br/>OpenAI: Authorization: Bearer"]
            HEADER["自定义 Header 规则处理"]
            LOG["日志记录"]
        end
...
本地路径到远程服务的映射规则定义，支持关联多个服务并按服务分组管理模型映射。

- 字段: `id`, `path`, `services`, `log_full_content`, `enabled`, `api_key`, `created_at`, `updated_at`
- `path`: 精确匹配的本地 URL 路径 (如 `/v1/chat/completions`)
- `services`: 关联的服务列表 (`Vec<AccessPointService>`)，第一个服务为当前激活服务
- `enabled`: 启用/禁用开关
- `log_full_content`: 是否记录完整的请求/响应体
- `api_key`: 接入点客户端鉴权密钥（可选），设置后客户端请求必须携带有效认证头才能通过代理转发

#### AccessPointService (值对象)

接入点关联的单个服务配置，包含该服务的模型映射规则。

- 字段: `service_id`, `model_mappings`
- `service_id`: 关联的 ApiService ID
- `model_mappings`: 该服务的模型映射规则列表

#### ModelMapping (值对象)

将客户端请求中的模型名映射为上游 API 服务的模型名。模型映射按服务分组，支持特殊值实现灵活的回退策略。

- 字段: `source`, `target`
- `source`: 客户端请求中的模型名，支持特殊值 `__other__` 表示匹配所有未显式映射的模型
- `target`: 上游 API 服务的模型名，支持特殊值 `__default__` 表示使用关联服务的 default_model
- 映射优先级: 接入点服务配置的映射表 > 服务模型列表透传 > default_model 回退
...
- 字段: `proxy_port` (默认 9876), `app_theme` (默认 "system"), `start_minimized` (默认 false), `services`, `access_points`, `log_settings`
- `start_minimized`: 静默启动开关，启用后应用启动时自动最小化到系统托盘，仅下次启动生效
...
- `update_app_theme` — 更新主题设置 (light/dark/system)
- `update_log_settings` — 更新日志设置 (最大条目 + 保留天数)
- `update_start_minimized` — 更新静默启动开关
...
5. 客户端鉴权: 如果 `access_point.api_key` 已设置，验证客户端请求中的认证头与接入点密钥匹配。验证方式是检查 `Authorization: Bearer {api_key}` 头。鉴权失败时返回 401 Unauthorized
6. 远程认证头注入: 根据 `service.api_type` 使用不同认证方式注入到转发请求:
   ...
   同时复制原始 Header (排除 `host`)，应用自定义 Header 规则
7. 请求转发: 使用 `reqwest` 发送异步 HTTP 请求
8. 日志记录: 写入请求/响应元数据和可选内容体到 SQLite (内容超过 10000 字符时截断)
...
├── App.tsx                # HashRouter + 4 个页面路由 + useEffect 主题初始化
├── types/                 # TypeScript 接口 (镜像 Rust Domain)
│   ├── api-service.ts     # ApiService 接口
│   ├── access-point.ts    # AccessPoint / ModelMapping 接口
...
│   └── useApiData.ts      # 通用 API 数据获取 Hook (loading + 错误处理 + 刷新回调)
├── components/            # 可复用组件
...
│   └── StatusBar.tsx      # 代理状态栏
├── utils/                 # 工具函数
│   ├── theme.ts           # 主题管理 (applyTheme / initTheme / onThemeChanged)
│   └── error.ts           # 错误消息提取 (getErrorMessage)
└── pages/                 # 业务页面
    ├── ServicesPage.tsx   # API 服务管理页面 (Table + Modal + Form)
...
sequenceDiagram
    ...
    PROXY->>PROXY: 检查接入点是否要求客户端鉴权
    alt access_point.api_key 已设置
        PROXY->>PROXY: 验证客户端 Authorization 头
        alt 鉴权失败
            PROXY-->>CLIENT: 401 Unauthorized
        end
    end
    PROXY->>PROXY: 根据 service.api_type 注入远程 API Key
    PROXY->>PROXY: 应用自定义 Header 规则
...
sequenceDiagram
    ...
    BACKEND->>YAML: 写回完整配置 (保留 proxy_port/app_theme)
    BACKEND-->>UI: 返回结果
```
...
sequenceDiagram
    ...
    participant TAU as Tauri IPC
    participant CS as ConfigService
    participant YAML as YAML 文件
    participant FE as 前端 DOM
    participant WS as Tauri Window

    UI->>TAU: invoke("update_app_theme", { theme: "dark" })
    TAU->>CS: update_app_theme("dark")
    CS->>YAML: 读取 → 修改 app_theme → 写回
    YAML-->>CS: Ok(())
    CS-->>TAU: Ok(())
    TAU-->>UI: 返回成功
    UI->>FE: applyTheme("dark")
    FE->>FE: document.body.setAttribute("theme-mode", "dark")
    FE->>WS: getCurrentWindow().setTheme("dark")

    Note over UI,WS: 跟随系统模式时注册 onThemeChanged 监听<br>系统主题变化自动同步 body theme-mode
```
...
- 管理后台鉴权依赖 OS 级别的用户隔离
- **接入点客户端鉴权**：每个接入点可设置独立的 `api_key`（可选），用于验证外部客户端的请求合法性。设置后客户端必须携带 `Authorization: Bearer {api_key}` 认证头才能通过代理转发
- **双层认证体系**：
  - 外层：接入点密钥验证客户端身份（可选，按接入点独立配置）
  - 内层：远程服务 API Key 由代理注入，对外部客户端透明隐藏
- 旧 YAML 配置自动兼容：`#[serde(default)]` 确保无 `api_key` 字段的旧配置不会报错

### 协议兼容性

代理转发在请求内容上不感知具体 AI API 协议，仅做 HTTP 透传，因此兼容 OpenAI-Compatible、Anthropic Messages API 等任意 HTTP API 协议。但在认证头注入层面做了协议感知：

- **Anthropic API**：使用 `x-api-key` + `anthropic-version` 请求头认证
- **OpenAI-Compatible API**：使用 `Authorization: Bearer` 请求头认证

认证头注入策略由 `ApiService.api_type` 字段控制，确保对远程服务的认证方式与目标 API 规范一致。
